...
input_chunk_length = 28  # Increased to capture monthly patterns [7, 28, 30, 365]
output_chunk_length = 7
num_filters = 64    # Increased to capture more complex patterns [4, 8, 16, 32, 64]
kernel_size = 3     # Kernel size for temporal convolutions [3, 5, ..., 25]
num_layers = 8  # Deep network for complex patterns
weight_norm = True # [False, True]
dilation_base = 2  # Exponential dilation for long-range dependencies [2, 3, 4]
dropout = 0.1  # Dropout rate for regularization [0.1, 0.2, 0.3, 0.4]

# Create logger and early stopping
logger = TensorBoardLogger("lightning_logs", name="tcn_model")
...
torch.save({
    ...
    'cov_scaler_state': cov_scaler.__getstate__(),
}, "models/tcn_model.pt")

# If you need to load the model later in a different session:
"""
...
# Load the state dict
checkpoint = torch.load("models/tcn_model.pt")
new_model._pytorch_module.load_state_dict(checkpoint['model_state_dict'])
new_scaler = SklearnMinMaxScaler()
new_scaler.__setstate__(checkpoint['scaler_state'])
new_cov_scaler = SklearnMinMaxScaler()
new_cov_scaler.__setstate__(checkpoint['cov_scaler_state'])
"""

# Load model and make predictions with covariates
# loaded_model = TFTModel.load("models/tcn_model.pt")
...
plt.plot(pred.time_index, pred.values(), 'r--', label='TCN Forecast', linewidth=2)
...
plt.title('TCN Model Forecast vs Actual (with uncertainty)', fontsize=14)
